...
export async function proxy(request: NextRequest) {
  ...
  response.headers.set("x-familyfinance-mw", "1");
  ...
  if (!supabaseUrl || !supabaseKey) {
    console.warn(
      "[proxy] NEXT_PUBLIC_SUPABASE_URL o chiave anon/publishable assenti → gate auth disattivato"
    );
  ...
  try {
    ...
    if (IS_DEV && error) {
      console.warn("[proxy] auth.getUser:", error.message);
    }
    ...
    if (!user && !isPublicPath(path)) {
      ...
      redirectResponse.headers.set("x-familyfinance-mw", "1");
